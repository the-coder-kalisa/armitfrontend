...
export default function BuildTrigger({ open, flow, setIsBuilt }) {
  ...
  async function handleBuild(flow) {
    try {
      if (isBuilding) {
        return
      }
  ...
  async function streamNodeData(flow) {
  ...
  async function enforceMinimumLoadingTime(startTime, minimumLoadingTime) {
