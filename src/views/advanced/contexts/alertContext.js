...
export function AlertProvider({ children }) {
  const [errorData, setErrorDataState] = useState({ title: "", list: [] })
  const [errorOpen, setErrorOpen] = useState(false)
  const [noticeData, setNoticeDataState] = useState({ title: "", link: "" })
  const [noticeOpen, setNoticeOpen] = useState(false)
  const [successData, setSuccessDataState] = useState({
    title: ""
  })
  const [successOpen, setSuccessOpen] = useState(false)
  const [notificationCenter, setNotificationCenter] = useState(false)
  const [notificationList, setNotificationList] = useState([])
  const pushNotificationList = notification => {
   ...
   */
  function setErrorData(newState) {
   ...
   */
  function setNoticeData(newState) {
    if (newState.title && newState.title !== "") {
   ...
   */
  function setSuccessData(newState) {
  ...
  function removeFromNotificationList(index) {
