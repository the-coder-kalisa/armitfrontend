...
export function normalCaseToSnakeCase(str) {
  return str
    .split(" ")
    .map((word, index) => {
      if (index === 0) {
...
export const isWrappedWithClass = (event, className) =>
  event.target.closest(`.${className}`)

export function groupByFamily(data, baseClasses, left, flow) {
  ...
  const checkBaseClass = template =>
    ...
    ((!excludeTypes.has(template.type) && baseClassesSet.has(template.type)) ||
      (template.input_types &&
        template.input_types.some(inputType => baseClassesSet.has(inputType))))
...
export function getRandomName(retry = 0, noSpace = false, maxRetries = 3) {
  const left = ADJECTIVES
  const right = NOUNS
...
export function getRandomKeyByssmm() {
  const now = new Date()
...
export function varHighlightHTML({ name }) {
 ...
 */
export function getChatInputField(flow, tabsState) {
 ...
 */
export function getPythonApiCode(flow, tweak, tabsState) {
 ...
 */
export function getCurlCode(flow, tweak, tabsState) {
  const flowId = flow.id
  const tweaks = buildTweaks(flow)
  const inputs = buildInputs(tabsState, flow.id)
 ...
 * @returns {string} - The python code
 */
export function getPythonCode(flow, tweak, tabsState) {
 ...
 */
export function getWidgetCode(flow, tabsState) {
  const flowId = flow.id
  const flowName = flow.name
  const inputs = buildInputs(tabsState, flow.id)
